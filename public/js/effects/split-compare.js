...
export function initSplitCompare(container, options = {}) {
	...
	return {
		...
		// Snap to a position immediately, bypassing the lerp.
		setPositionImmediate(percent) {
			if (animationId) {
		...
		// One-shot tween from `from` → `to`. Bypasses the lerp so the timing
		// is exact. Returns a Promise that resolves when the tween completes.
		autoplay({ from = 0, to = 100, duration = 1500, easing } = {}) {
			// Default: smoothstep (easeInOut)
			const ease = easing || ((t) => t * t * (3 - 2 * t));
			// Cancel any in-flight lerp so we own the divider for the duration.
			if (animationId) {
				cancelAnimationFrame(animationId);
				animationId = null;
			}
