...
export default function UploadDialog({ onFileSelected, onLinkSelected, onClose }: UploadDialogProps) {
  ...
  return (
    <div className="wsv2-modal-backdrop" onMouseDown={onClose} role="presentation">
      <div
        className="wsv2-modal wsv2-upload-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="wsv2-upload-dialog-title"
        onMouseDown={e => e.stopPropagation()}
      >
        <div className="wsv2-modal-header wsv2-upload-dialog-header">
          <div className="wsv2-modal-heading">
            <h2 className="wsv2-modal-title wsv2-upload-dialog-title" id="wsv2-upload-dialog-title">{t('upload.title')}</h2>
            <p className="wsv2-modal-subtitle wsv2-upload-dialog-subtitle">{t('upload.subtitle')}</p>
          </div>
          <button className="wsv2-modal-close" onClick={onClose} aria-label={tc('actions.close')}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
        <div className="wsv2-modal-body wsv2-upload-dialog-body">
          <div
            className={`wsv2-upload-dialog-dropzone${dragover ? ' is-dragover' : ''}`}
          ...
          >
            <div className="wsv2-upload-dialog-dropzone-icon">
            ...
            <p className="wsv2-upload-dialog-dropzone-text">{t('upload.dropText')}</p>
            <p className="wsv2-upload-dialog-dropzone-hint">{t('upload.dropHint')}</p>
          </div>

          <div className="wsv2-upload-dialog-divider"><span>{t('upload.fromCloud')}</span></div>

          <div className="wsv2-upload-dialog-cloud-sources">
            <button
              type="button"
              className="wsv2-upload-dialog-cloud-btn"
          ...
          <div className="wsv2-upload-dialog-divider"><span>{t('upload.fromLink')}</span></div>

          <form className="wsv2-upload-dialog-link-form" onSubmit={handleLinkSubmit}>
            <input
              type="url"
              className="wsv2-upload-dialog-link-input"
            ...
            <button
              type="submit"
              className="wsv2-upload-dialog-link-btn"
              disabled={!linkInput.trim() || linkSubmitting}
            >
              {t('upload.addLink')}
            </button>
          </form>
          {linkError && <p className="wsv2-upload-dialog-link-error" role="alert">{linkError}</p>}
