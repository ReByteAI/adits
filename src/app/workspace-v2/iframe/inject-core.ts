/** Typed reference for the inject script. The runtime copy is
 *  `server/vm-bin/adits-file-server/inject.js` (go:embed-ed by the
 *  VM's Go file-server binary and served at `/_adits/inject.js`).
 *  This file is kept hand-aligned with inject.js so the TypeScript
 *  compiler sees the IIFE text and the bridge-side types can be
 *  cross-referenced during review.
 *
 *  **To ship a change, edit BOTH files, rebuild the Go binary, and
 *  bump `INSTALL_VERSION` in `server/file-server/install.ts`.**
 *
 *  Modes supported:
 *    - 'edit'    — design-mode element picker, stamp `data-dm-ref="N"`
 *    - 'comment' — comment-pin element picker, stamp `data-cc-id="cc-N"`
 *    - present   — separate orthogonal flag ('tab' | 'fullscreen')
 *                  that installs tap-to-advance + the `__omPresent`
 *                  markers read by the deck's own chrome-hide code.
 *  `data-dm-overlay` is the shared opt-out marker that walks both
 *  click-filter and stamps the outline div itself.
 *
 *  Full message shapes live in `iframe/messages.ts` (single source of
 *  truth for both the host controllers and this script).
 */
export const INJECT_SOURCE = `
(function () {
