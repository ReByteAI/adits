/**
 * Executors: the three agent CLIs Adits drives. Shared between the hosted
 * Rebyte path (where `executor` is a field on Rebyte's `POST /tasks` body)
 * and the local single-user path (where the executor value becomes the
 * name of the CLI binary to spawn).
 *
 * `opencode` was here as a fourth option but was dropped — it didn't add
 * anything Claude/Codex/Gemini don't already cover, and keeping it around
 * meant a UI item that didn't map to a local CLI.
 */
 ...
 *  the user-facing display name shown in the picker. */
...
export const EXECUTOR_LOGO_VIEWBOX: Record<ExecutorType, string> = {
  claude: '0 0 24 24',
  gemini: '0 0 24 24',
  codex: '0 0 24 24',
}

export const EXECUTOR_BRAND_COLORS: Record<ExecutorType, string> = {
  claude: '#d97706',
  gemini: '#3186FF',
  codex: '#000000',
}

/** Default executor if the UI hasn't picked one yet. */
export const DEFAULT_EXECUTOR: ExecutorType = 'claude'
